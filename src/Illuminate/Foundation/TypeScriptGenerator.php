...
class TypeScriptGenerator
{
    ...
    {
        $typescript = $reflections->map(fn (ReflectionClass $reflection) => $this->makeInterface($reflection))
            ->whereNotNull()
        ...
        return Str::replace(PHP_EOL . PHP_EOL . '}', PHP_EOL . '}', $typescript);
    }

    protected function makeInterface(ReflectionClass $reflection): ?string
    {
        $generator = collect($this->generators)
            ->filter(fn (string|array $generator, string $baseClass) => $reflection->isSubclassOf($baseClass))
            ->values()
            ->first();
        ...
        if (is_array($generator)) {
            return (new $generator['name']($generator['settings']))->generate($reflection);
        }
            ...
            ->flatMap(function (string $path, string $namespace) {
                return collect((new Finder)->in($path)->name('*.php')->files())
                    ...
                    ->filter(function (string $className) {
                        if (Str::endsWith($className, 'Test')) return false;
